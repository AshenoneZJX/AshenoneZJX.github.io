...
/* 黑色主题背景 */
.navigation.active{ 
  background-image: url("./assets/img/back/aitx6-0og08.webp") !important;
...
.navigation {
  ...
  height: 600px;
  border-radius: 20px;
  background-image: url("./assets/img/back/a58cbbffd2aa49c1b1e99990be912f30.jpg");
...
/* 两个连续的.XXX 表示:选择在.navigation元素内部的.buttom元素 */
...
.nav-titleBox .buttom::after{
  ...
  border-left: 8px solid var(--titleColor);
...
.me-hover span{
  ...
  display: block;
  font: 14px ;
  border-right: .08em solid var(--headerFont);
  color: var(--headerFont); 
  /* 宽度为个数*2ch */
  width: 15ch; 
  white-space: nowrap;
  overflow: hidden;
  animation: typing 3s steps(14,end), blink-caret .3s step-end infinite alternate;
}
...
.content .borderbotm {
  border-style: dashed;
  border-bottom: 1px dashed var(--borderline);
  height: 0px;
  margin: 35px 0;
}
...
/* 全屏加载遮罩层：固定定位，覆盖整个视口，黑色半透明背景，最高层级显示 */
.lodding-wrap{
  position: fixed;
  left: 0px;
  right: 0px;
...
.zz{
  position: relative;
  /* z-index 属性设置定位元素及其后代元素或 flex 项目的 Z 轴顺序。z-index 值较大的元素会覆盖较小的元素。 */
  z-index: 1;
}
...
.music-wrap .music-icon {
  font-size: 18px;
  margin-right: 8px;
  animation: pulse 2s infinite;
}

.music-wrap .music-title {
  font-weight: 500;
  white-space: nowrap;
}
